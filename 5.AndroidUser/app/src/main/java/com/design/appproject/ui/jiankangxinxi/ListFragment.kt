...
import com.design.appproject.ui.jiankangxinxi.ListFilterDialog
...
import com.design.appproject.databinding.JiankangxinxicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.jiankangxinxi.ListModel
import com.design.appproject.ui.jiankangxinxi.ListAdapter
 ...
 * 健康信息列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_JIANKANGXINXI)
class ListFragment : BaseBindingFragment<JiankangxinxicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("健康信息",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("jiankangxinxi","新增"))|| Utils.isAuthFront("jiankangxinxi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_JIANKANGXINXI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_JIANKANGXINXI)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("jiankangxinxi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_JIANKANGXINXI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun JiankangxinxicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("jiankangxinxi", requestParams)
        }.otherwise {
                mListModel.list("jiankangxinxi", requestParams)
        }
    }
}