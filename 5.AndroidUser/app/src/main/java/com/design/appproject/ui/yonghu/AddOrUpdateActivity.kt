...
class AddOrUpdateActivity:BaseBindingActivity<YonghuaddorupdateLayoutBinding>() {
    ...
    private fun YonghuaddorupdateLayoutBinding.submit() {
        mYonghuItemBean.zhanghao = zhanghaoEt.text.toString()
        mYonghuItemBean.xingming = xingmingEt.text.toString()
        mYonghuItemBean.mima = mimaEt.text.toString()
        nianlingEt.inputType = InputType.TYPE_CLASS_NUMBER
        mYonghuItemBean.nianling = nianlingEt.text.toString().toInt()
        mYonghuItemBean.shouji = shoujiEt.text.toString()
        if(mYonghuItemBean.zhanghao.isNullOrEmpty()){
            "账号不能为空".showToast()
            return
        }
        ...
        RegexUtils.isMobileExact(mYonghuItemBean.shouji).no {
            "手机应输入手机格式".showToast()
            return
        }
        addOrUpdate()
    ...
    private fun YonghuaddorupdateLayoutBinding.setData(){
        if (mYonghuItemBean.zhanghao.isNotNullOrEmpty()){
            zhanghaoEt.setText(mYonghuItemBean.zhanghao.toString())
        }
        if (mYonghuItemBean.xingming.isNotNullOrEmpty()){
            xingmingEt.setText(mYonghuItemBean.xingming.toString())
        }
        ...
        if (mYonghuItemBean.nianling>=0){
            nianlingEt.setText(mYonghuItemBean.nianling.toString())
        }
        if (mYonghuItemBean.shouji.isNotNullOrEmpty()){
            shoujiEt.setText(mYonghuItemBean.shouji.toString())
        }
    }
}