...
import com.design.appproject.ui.jiankangfenxi.ListFilterDialog
...
import com.design.appproject.databinding.JiankangfenxicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.jiankangfenxi.ListModel
import com.design.appproject.ui.jiankangfenxi.ListAdapter
 ...
 * 健康分析列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_JIANKANGFENXI)
class ListFragment : BaseBindingFragment<JiankangfenxicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("健康分析",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("jiankangfenxi","新增"))|| Utils.isAuthFront("jiankangfenxi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_JIANKANGFENXI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("jiankangfenxi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_JIANKANGFENXI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun JiankangfenxicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("jiankangfenxi", requestParams)
        }.otherwise {
                mListModel.list("jiankangfenxi", requestParams)
        }
    }
}