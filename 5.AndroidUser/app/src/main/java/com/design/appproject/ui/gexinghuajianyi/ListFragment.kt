...
import com.design.appproject.ui.gexinghuajianyi.ListFilterDialog
...
import com.design.appproject.databinding.GexinghuajianyicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.gexinghuajianyi.ListModel
import com.design.appproject.ui.gexinghuajianyi.ListAdapter
 ...
 * 个性化建议列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_GEXINGHUAJIANYI)
class ListFragment : BaseBindingFragment<GexinghuajianyicommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("账号","zhanghao")/*查询索引*/
    ...
    override fun initEvent() {
        setBarTitle("个性化建议",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("gexinghuajianyi","新增"))|| Utils.isAuthFront("gexinghuajianyi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_GEXINGHUAJIANYI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_GEXINGHUAJIANYI)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("gexinghuajianyi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_GEXINGHUAJIANYI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun GexinghuajianyicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("zhanghao","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("zhanghao")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
                mListModel.page("gexinghuajianyi", requestParams)
        }.otherwise {
                mListModel.list("gexinghuajianyi", requestParams)
        }
    }
}