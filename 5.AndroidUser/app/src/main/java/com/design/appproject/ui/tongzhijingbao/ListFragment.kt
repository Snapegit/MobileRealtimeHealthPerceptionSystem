...
import com.design.appproject.ui.tongzhijingbao.ListFilterDialog
...
import com.design.appproject.databinding.TongzhijingbaocommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.tongzhijingbao.ListModel
import com.design.appproject.ui.tongzhijingbao.ListAdapter
 ...
 * 通知警报列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_TONGZHIJINGBAO)
class ListFragment : BaseBindingFragment<TongzhijingbaocommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("通知警报",mHasBack)
        setBarColor("#FFFFFF","black")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("tongzhijingbao","新增"))|| Utils.isAuthFront("tongzhijingbao","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_TONGZHIJINGBAO).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_TONGZHIJINGBAO)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("tongzhijingbao",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_TONGZHIJINGBAO,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun TongzhijingbaocommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("tongzhijingbao", requestParams)
        }.otherwise {
                mListModel.list("tongzhijingbao", requestParams)
        }
    }
}