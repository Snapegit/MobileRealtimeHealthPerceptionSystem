        ...
        mHomeViewModel.newsListLiveData.observeKt {
            it.getOrNull()?.let {
                ...
                it.data.list.forEachIndexed { index, newsListBean ->
                    newsLl.addView(creatNewItemView(newsListBean,index))
                }
            }
        }
    }

    private fun creatNewItemView(newsListBean: NewsItemBean,index:Int) =LayoutInflater.from(context).inflate(R.layout.item_news_layout,binding.newsLl,false).apply{
        findViewById<ViewGroup>(R.id.news_top_fl).isVisible = index == 0
        findViewById<ImageView>(R.id.news_iv).load(context, newsListBean.picture)
        findViewById<TextView>(R.id.news_top_title_tv).text = newsListBean.title
        findViewById<ViewGroup>(R.id.news_fl).isVisible = index>0
